...
internal class OnIncomingConnectionAttemptMsg(private val msg: SessionDescriptionMsg) : Event {
    ...
    override fun process() {
        if(!SharedStore.areIncomingConnectionsAllowed()){
        ...
        } else if(!incomingCrolangNodesCallbacks!!.onConnectionAttempt(msg.from, msg.platformFrom, msg.versionFrom)) {
    ...
    private fun onIncomingConnectionsNotAllowed(){
        ...
        if(socket != null && socket!!.connected()){
            ...
            incomingConnectionsNotAllowedMsg.platformFrom = SharedStore.dependencies!!.myPlatform
            incomingConnectionsNotAllowedMsg.versionFrom = SharedStore.dependencies!!.myVersion
            ...
            socket!!.emit(INCOMING_CONNECTIONS_NOT_ALLOWED, parser.toJson(incomingConnectionsNotAllowedMsg)){}
        }
    }
    ...
    private fun onConnectionAttemptRefusedByUserDefinedCallback(){
        ...
        if(socket != null && socket!!.connected()){
            ...
            connectionRefusalMsg.platformFrom = SharedStore.dependencies!!.myPlatform
            connectionRefusalMsg.versionFrom = SharedStore.dependencies!!.myVersion
            connectionRefusalMsg.to = msg.from
            connectionRefusalMsg.from = localNodeId
            connectionRefusalMsg.sessionId = msg.sessionId
            socket!!.emit(CONNECTION_REFUSAL, parser.toJson(connectionRefusalMsg)){}
        }
    }
    ...
    private fun onConnectionAttemptAccepted(){
        ...
        if(rtcConfiguration == null){
            ...
            val newNode = ResponderNode(
                rtcConfiguration!!, remoteNodeId, msg.sessionId, incomingCrolangNodesCallbacks!!
            )
            ...
            newNode.peer.setRemoteDescription(
                offer,
                onSuccess = { SharedStore.dependencies!!.eventLoop.postEvent(OnRemoteDescriptionSetSuccessfullyResponderNode(remoteNodeId)) },
                onFailure = { SharedStore.dependencies!!.eventLoop.postEvent(OnRemoteDescriptionSetFailureResponderNode(remoteNodeId, it)) }
            )
        }
    }
...
internal class OnRemoteDescriptionSetSuccessfullyResponderNode(
    ...
    override fun onNodeFound(node: ResponderNode) {
        ...
        node.peer.createAnswer(
            onSuccess = { SharedStore.dependencies!!.eventLoop.postEvent(OnAnswerCreationSuccessResponderNode(remoteNodeId, it)) },
            onFailure = { SharedStore.dependencies!!.eventLoop.postEvent(OnAnswerCreationFailureResponderNode(remoteNodeId, it)) }
        )
    }
 ...
 *
 * @param remoteNodeId The ID of the remote initiator node that the responder node is attempting to connect to.
 * @param description The CrolangP2PRTCSessionDescription object representing the answer created by the responder node.
 */
internal class OnAnswerCreationSuccessResponderNode(
    remoteNodeId: String, private val description: CrolangP2PRTCSessionDescription
) : ResponderNodeAbstractEvent(remoteNodeId) {

    override fun onNodeFound(node: ResponderNode) {
        logger.debugInfo("answer created on local ResponderNode $remoteNodeId, setting local description")
        node.peer.setLocalDescription(
            description,
            onSuccess = { SharedStore.dependencies!!.eventLoop.postEvent(OnLocalDescriptionSetSuccessResponderNode(remoteNodeId, description)) },
            onFailure = { SharedStore.dependencies!!.eventLoop.postEvent(OnLocalDescriptionSetFailureResponderNode(remoteNodeId, it)) }
        )
    }
...
internal class OnLocalDescriptionSetSuccessResponderNode(
    ...
    override fun onNodeFound(node: ResponderNode) {
        ...
        AgnosticRTCSessionDescription.adaptConcrete(description)?.let {
...
internal class OnP2PConnectionStateChangeResponderNode(
    remoteNodeId: String, private val state: CrolangP2PRTCPeerConnectionState
) : ResponderNodeAbstractEvent(remoteNodeId) {

    override fun onNodeFound(node: ResponderNode) {
        logger.debugInfo("peer connection to responder node $remoteNodeId state changed: $state")
        if(state == CrolangP2PRTCPeerConnectionState.DISCONNECTED || state == CrolangP2PRTCPeerConnectionState.FAILED){
...
internal class OnDataChannelStateChangeResponderNode(
    remoteNodeId: String, private val channel: CrolangP2PRTCDataChannel, private val state: CrolangP2PRTCDataChannelState
) : ResponderNodeAbstractEvent(remoteNodeId) {

    override fun onNodeFound(node: ResponderNode) {
        logger.debugInfo("data channel state changed to $state with remote InitiatorNode $remoteNodeId")
        if(state == CrolangP2PRTCDataChannelState.OPEN && node.isNegotiating(node.state)){
            node.connectionTimeoutTimer.cancel()
            node.dataChannel = channel
            ...
            node.forceClose(NodeState.NEGOTIATION_ERROR)
        } else if(state == CrolangP2PRTCDataChannelState.CLOSED && node.state == NodeState.CONNECTED){
            node.forceClose(NodeState.DISCONNECTED)
        }
    }
