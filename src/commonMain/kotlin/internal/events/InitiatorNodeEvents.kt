...
internal abstract class InitiatorNodeAbstractEvent(protected val remoteNodeId: String) : Event {
     ...
     * @param failureErr A nullable error message describing the failure cause.
     */
...
internal class OnInitiatorNodeReadyToCreateOffer(remoteNodeId: String) : InitiatorNodeAbstractEvent(remoteNodeId) {
    ...
    override fun onNodeFound(node: InitiatorNode) {
        ...
        node.peer.createOffer(
            onSuccess = { SharedStore.dependencies!!.eventLoop.postEvent(OnOfferCreatedSuccessfullyInitiatorNode(remoteNodeId, it)) },
            onFailure = { SharedStore.dependencies!!.eventLoop.postEvent(OnOfferCreatedFailureInitiatorNode(remoteNodeId, it)) },
        )
    }
}
...
internal class OnOfferCreatedSuccessfullyInitiatorNode(
    ...
    override fun onNodeFound(node: InitiatorNode) {
        ...
        node.peer.setLocalDescription(
            description,
            onSuccess = { SharedStore.dependencies!!.eventLoop.postEvent(OnOfferSetSuccessfullyInitiatorNode(remoteNodeId, description)) },
            onFailure = { SharedStore.dependencies!!.eventLoop.postEvent(OnOfferSetFailureInitiatorNode(remoteNodeId, it)) }
        )
    }
}
...
internal class OnOfferSetSuccessfullyInitiatorNode(
    ...
    override fun onNodeFound(node: InitiatorNode) {
        ...
        AgnosticRTCSessionDescription.adaptConcrete(description)?.let { agnosticDescription ->
            ...
            node.sendSocketMsg(CONNECTION_ATTEMPT, msg) { response: String ->
                if(!SocketResponses.isOk(response)){
                    SharedStore.dependencies!!.eventLoop.postEvent(OnConnectionAttemptNotDeliveredInitiatorNode(remoteNodeId, response))
                }
            }
        } ?: run {
            connectionNegotiationError(
...
internal class OnIncomingP2PConnectionAcceptanceMsg(
    ...
    override fun onNodeFound(node: InitiatorNode) {
        ...
        node.peer.setRemoteDescription(
            ...
            onSuccess = { SharedStore.dependencies!!.eventLoop.postEvent(OnRemoteDescriptionSetSuccessfullyInitiatorNode(message.from)) },
            onFailure = { SharedStore.dependencies!!.eventLoop.postEvent(OnRemoteDescriptionSetFailureInitiatorNode(message.from, it)) }
        )
    }
...
internal class OnDataChannelStateChangeInitiatorNode(
    remoteNodeId: String, private val state: CrolangP2PRTCDataChannelState
) : InitiatorNodeAbstractEvent(remoteNodeId) {
